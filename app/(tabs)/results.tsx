...
import {
    computeTotalBytes,
    formatMB,
} from "../../src/services/scan-orchestrator";
...
function ViewerImageItemBase({ item }: { item: ScoredResult }) {
...
export default function ResultsScreen(): React.JSX.Element {
  ...
  const [activeFilter, setActiveFilter] = useState<ConfidenceLevel | "ALL">(
    "ALL",
  );
  ...
  const hasPendingRemoval = state.pendingDeletions.length > 0;
  ...
  const handleDelete = useCallback(() => {
    if (selectedCount === 0 || hasPendingRemoval) return;
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy);
    router.push("/delete-confirm");
  }, [hasPendingRemoval, selectedCount, router]);

  const handleShare = useCallback(
    async (item: ScoredResult) => {
      if (isSharing) return;
      setIsSharing(true);
      try {
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: ScoredResult; index: number }) => {
        ...
        >
          ...
          {item.confidence !== "CLEAN" && (
            <View
              style={[
                styles.badge,
                { backgroundColor: BADGE_COLORS[item.confidence] },
              ]}
            >
  ...
  const renderViewerFooter = useCallback(
        ...
        <View style={styles.viewerBottomBar}>
          <View style={styles.viewerInfo}>
            <Text style={styles.viewerInfoText}>
              {formatMB(item.asset.fileSize)} · {item.asset.width}×
              {item.asset.height}
            </Text>
          ...
          <View style={styles.viewerActions}>
            ...
            <Pressable
              ...
              style={[
                ...
                isSelected && styles.viewerSelectBtnActive,
              ]}
            >
              <MaterialIcons
                name={isSelected ? "check-circle" : "radio-button-unchecked"}
                size={22}
                color={isSelected ? Colors.primaryContainer : "#FFF"}
              />
              <Text
                style={[
                  styles.viewerSelectText,
                  isSelected && styles.viewerSelectTextActive,
                ]}
              >
    ...
    [handleShare, handleToggle, isSharing, state.selectedIds],
  );
    ...
    <SafeAreaView style={styles.safeArea} edges={["left", "right", "bottom"]}>
      ...
      <View style={styles.summaryStrip}>
        <Text style={styles.summaryTitle}>
          {totalItems} {isSourceMode ? "image" : "spam image"}
          {totalItems !== 1 ? "s" : ""}
      ...
      <View style={styles.actionBar}>
        ...
        <Pressable
          ...
          disabled={selectedCount === 0 || hasPendingRemoval}
          style={({ pressed }) => [
            styles.deleteButton,
            pressed && styles.deleteButtonPressed,
            (selectedCount === 0 || hasPendingRemoval) &&
              styles.deleteButtonDisabled,
          ]}
        >
          <Text style={styles.deleteButtonText}>
            {hasPendingRemoval ? "Removal pending" : `Delete ${selectedCount}`}
          </Text>
        </Pressable>
      </View>
